...
import { INITIAL_COUPONS } from '@shared/data/mockData';
  ...
  return (
    <div className="space-y-6 pb-16">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h1 className="text-2xl font-black text-[#10213F]">Coupons & Offer Management</h1>
          <p className="text-xs text-[#64748B]">Configure promotional discount codes</p>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        {coupons.map((c, i) => (
          <div key={i} className="glass-card p-5 rounded-2xl border border-[#E6ECF5] flex items-center justify-between">
            <div>
              <div className="flex items-center gap-2">
                <span className="font-mono font-bold text-[#1264F5] text-lg">{c.code}</span>
                <span className="text-xs bg-[#F0F6FF] text-[#1264F5] px-2 py-0.5 rounded-full font-bold">
                  {c.type === 'flat' ? `₹${c.discount} OFF` : `${c.discount}% OFF`}
                </span>
              </div>
              <p className="text-xs text-[#334155] mt-1">{c.description}</p>
              <span className="text-[10px] text-[#64748B] block mt-1">Valid Till: {c.expiry}</span>
            </div>
