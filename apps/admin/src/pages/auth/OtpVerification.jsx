...
export const OtpVerification = () => {
  ...
  return (
    <div className="min-h-screen flex flex-col justify-center items-center p-4 bg-white text-slate-100 relative overflow-hidden">
      <div className="w-full max-w-md glass-panel border border-[#CBD5E1]/60 rounded-3xl p-6 sm:p-8 shadow-2xl relative z-10">
        <button
          onClick={() => navigate(-1)}
          className="flex items-center gap-1 text-xs text-[#64748B] hover:text-white mb-4"
        >
          ...
          <div className="flex items-center justify-between gap-2">
            {otp.map((digit, idx) => (
              <input
                ...
                className="w-12 h-14 bg-[#F8FAFC] border border-[#CBD5E1] focus:border-cyan-500 focus:ring-2 focus:ring-cyan-500/20 text-center text-xl font-bold text-white rounded-xl outline-none transition-all"
              />
            ))}
          </div>
        ...
        {/* Resend & Timer */}
        <div className="text-center mt-6 pt-4 border-t border-[#E6ECF5]/80 text-xs">
          {canResend ? (
            <button
