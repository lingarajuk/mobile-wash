  ...
  return (
    <div className="space-y-6 pb-16">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h1 className="text-2xl font-black text-[#10213F]">Washing Service Management</h1>
          <p className="text-xs text-[#64748B]">Configure wash packages, pricing and durations</p>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
        {services.map((srv) => (
          <div key={srv.id} className="glass-card p-5 rounded-2xl border border-[#E6ECF5] space-y-3 flex flex-col justify-between">
            <div>
              <div className="flex justify-between items-start">
                <h4 className="text-base font-black text-[#10213F]">{srv.name}</h4>
                <span className="text-[#1264F5] font-extrabold text-sm">₹{srv.price}</span>
              </div>
              <p className="text-xs text-[#64748B] line-clamp-2 mt-1">{srv.description}</p>
              <span className="text-[10px] text-[#64748B] block mt-2">Duration: {srv.duration}</span>
            </div>

            <div className="pt-3 border-t border-[#E6ECF5] flex justify-end gap-2">
              <button
                onClick={() => {
