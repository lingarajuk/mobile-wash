  ...
  return (
    <div className="space-y-6 pb-16">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h1 className="text-2xl font-extrabold text-white">Customer Accounts Management</h1>
          <p className="text-xs text-slate-400">View customer profile history and manage account active statuses</p>
        </div>
      </div>
            ...
            <tbody className="divide-y divide-slate-800/80">
              {filtered.map(c => (
                <tr key={c.id} className="hover:bg-slate-900/40">
                  ...
                  <td className="p-3.5 text-slate-300">{c.phone}<br/><span className="text-[10px] text-slate-500">{c.email}</span></td>
                  <td className="p-3.5 text-slate-300">{c.city}</td>
                  <td className="p-3.5 font-bold text-cyan-400">{c.totalBookings} Washes</td>
